...
async function startServer() {
  ...
  app.post('/api/payments/create-preference', async (req, res) => {
    ...
    try {
      const { trainerAccessToken, studentEmail, planName, price, studentId, trainerId, metadata } = req.body;
      ...
      const response = await preference.create({
        body: {
          ...
          external_reference: JSON.stringify({ studentId, trainerId, planName, durationDays: metadata?.durationDays || 30 }),
